...
function reverseArr(arr) {
    reverse = []
    arr.forEach(el => reverse.unshift(el)) // adds each element to the front of the arr
    return reverse
}
...
function objectKeysToArr(object) {
    keys = []
    for (let key in object) { // "in" allows us to loop through the keys of objects
        keys.push(key)
    }
    return keys
}
...
function combineObjects(object1, object2) {
    return {
        ...object1, // spread those objects into a new object
        ...object2
    }
}
